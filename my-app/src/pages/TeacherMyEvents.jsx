...
const TeacherMyEvents = ({ events, theme }) => {
  ...
  const styles = {
    ...
    filterBtn: (isActive) => ({
      ...
      backgroundColor: isActive ? '#2563EB' : (isDark ? '#1e293b' : '#fff'),
    ...
    statusBadge: (status) => ({
      ...
      backgroundColor: status === 'published' ? '#dcfce7' : status === 'draft' ? '#fef3c7' : status === 'completed' ? '#EFF6FF' : '#f1f5f9',
      color: status === 'published' ? '#166534' : status === 'draft' ? '#92400e' : status === 'completed' ? '#1E3A8A' : '#475569'
    }),
